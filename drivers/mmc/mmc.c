 ...
 */
// SD卡中有扩展的CSD寄存器，card special data，
// 低版本的规范中有csd寄存器，
// 高版本中csd寄存器不够用，所以需要extended CSD来满足要求。
// 这个函数的功能就是读取高版本的SD卡中的扩展csd寄存器的
static int mmc_read_ext_csd(struct mmc *host)
{
	int err;
	u8 *ext_csd;
	unsigned int ext_csd_struct;
	// host指的是s5pv210中的SD卡控制器
	// host->version指的是SD卡控制器的不同版本
	...
	// 代码对版本号进行了判断，并且如果版本号大于5就会报错且函数退出。这就是问题所正
	// 问题就是: 我们使用的iNand的版本号大于5，而uboot代码本身不处理版本号大于5的卡，因此出错了。
	// 怎么解决; 第一，换卡；第二，软件修复。
	// 我们的eMMC的版本号是5.0，所以我们这里if语句改为大于8即可
	//if (ext_csd_struct > 5) {
	if (ext_csd_struct > 8) {					/* sxh changed */
		// 根据CRT显示打印出来的信息定位到这里
...
int mmc_initialize(bd_t *bis)
{
	...
	print_mmc_devices(',');
#endif
	// 0表示内部iNand；1表示SD卡，我们实际uboot中初始化的是iNand，用的是SD0
