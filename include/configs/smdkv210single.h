...
#define MEMORY_BASE_ADDRESS		0x30000000			/* sxh added*/
...
// 如果使能了MMU 则需要进行虚拟地址到物理地址的映射
// virt_to_phy_smdkc110这个函数就是处理虚拟地址到物理地址的映射
...
// CONFIG_DM9000_BASE是DM9000网卡通过SROM bank映射到SOC中地址空间中的地址。这个地址的值取决于硬件
// 接到了哪个bank，这个bank的基地址是SOC自己定义好的。譬如我们这里接到了bank1上，bank1的基地址是
// 0x88000000。
//#define CONFIG_DM9000_BASE		(0xA8000000)
#define CONFIG_DM9000_BASE		(0x88000300)			/* sxh changed */
// DM9000_IO表示访问芯片IO的基地址，直接就是CONFIG_DM9000_BASE
#define DM9000_IO				(CONFIG_DM9000_BASE)
#if defined(DM9000_16BIT_DATA)
//#define DM9000_DATA				(CONFIG_DM9000_BASE+2)
// +4的原因是二进制100，地址线的第二根，对应addr2
// 主要原因是数据线和地址线复用，当addr2为高电平时采用数据线，反之是地址线
// 这里正好addr2是4，为高电平，使用的是数据线
// DM9000_DATA表示我们访问数据时的基地址，因为DM9000芯片的cmd引脚接到了ADDR2，因此这里要+4(0b100，对应ADRR2)
#define DM9000_DATA				(CONFIG_DM9000_BASE+4)	/* sxh changed */
#else
#define DM9000_DATA				(CONFIG_DM9000_BASE+1)
#endif
#endif
// 本来这样配置完了，重新编译运行网卡就应该工作了。但是实际测试发现不工作，要怎么样修改呢?
// 修改方式是将CONFIG_DM9000_BASE改成0x88000300就工作了。
/*
...
// 修改串口
// #define CONFIG_SERIAL1          1	/* sxh changed */
...
#define CONFIG_IPADDR		192.168.1.10		/* sxh changed */
#define CONFIG_SERVERIP		192.168.1.141		/* sxh changed */
#define CONFIG_GATEWAYIP	192.168.1.1			/* sxh changed */
// 更改完成后，如果环境变量还是原来的，正常。因为原来uboot执行过saveenv，因此环境变量已经被保存到
// iNand中的ENV分区中去了。uboot启动后校验时iNand的ENV分区中的环境变量是正确的，因此会优先加载。
// 我们在uboot源代码中修改的只是默认的环境变量。解决方案是擦除掉iNand中那一份环境变量，然后迫使
// uboot启动时使用uboot代码中自带的默认的这一份环境变量，就可以看到了。
// 可以使用mmc write 0 30000000 11# 32 (表示将DDR的0x30000000开头的一段内存中的内容写入iNand中的
// 第17个扇区开始的32个扇区，写入长度是32个扇区长度(16KB))
...
#define CFG_PROMPT              "Ethan210 # "		/* sxh changed */
...
// 移植的时候如果有关时间部分出现问题，则需要在这里对时钟进行修改。
...
#if defined(CONFIG_MCP_SINGLE)
// 将DDR端口0地址配置为30000000开头
// 更改有2个目的: 第一是让大家体验内存配置的更改过程；第二是3开头的地址和DRAM bank1上40000000开头
// 的地址就连起来了。这样我们就得到了地址连续的512MB内存，而原来我们得到的512MB内存地址是断续的。
// 根据裸机中将DDR初始化部分的课程，和uboot前面分析中DDR初始化部分的代码的课程，得出结论就是: DDR的
// 初始化代码部分是在lowlevel.S中写的，是不动的。代码部分就是对相应寄存器做相应值的初始化；要动的是
// 值，而uboot为了具有可移植性把值都宏定义在include/configs/xxx.h中了。因此我们只需要去这个配置头文
// 件中更改配置值即可。这里修改的是DDR的寄存器的值(寄存器的值定义在这个文件，为了方便移植)
// 寄存器的值改了后相当于是硬件配置部分做了更改。但是我们uboot中DDR相关的一些软件配置值还没更改，
// 还在原来位置，所以要去修改。

// 移植时，DDR部分出问题，需要在这里修改(这里修改的是DDR寄存器配置，这里要结合数据手册DDR部分进行修改)
// #define DMC0_MEMCONFIG_0	0x20E01323	// MemConfig0	256MB config, 8 banks,Mapping Method[12:15]0:linear, 1:linterleaved, 2:Mixed
#define DMC0_MEMCONFIG_0	0x30F01323	/* sxh added*/
...
// DDR移植时遇到问题，需要注意的地方有DDR地址配置部分，DDR寄存器配置部分，虚拟地址到物理地址映射部分

// 移植时，DDR部分出问题，需要在这里修改(这里修改的是DDR地址配置)
#define CONFIG_NR_DRAM_BANKS    2          /* we have 2 bank of DRAM */
//#define SDRAM_BANK_SIZE         0x20000000    /* 512 MB */
#define SDRAM_BANK_SIZE         0x10000000    /* 256 MB */		/* sxh added*/
...
#define CONFIG_IDENT_STRING		" for ethan210"			/* sxh added*/
...
#define CFG_UBOOT_BASE			0x23e00000		/* sxh added */
#endif
